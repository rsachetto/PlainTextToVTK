...
bool new_vtk_unstructured_grid_from_file(struct vtk_unstructured_grid **vtk_grid, char *file, bool clip_with_plain,
    ...
    sds *file_lines =  read_lines(file);

    ptrdiff_t lines_number = arrlen(file_lines);

    if(lines_number == 0) return false;

    for(ptrdiff_t i = 0; i < lines_number; i++) {

        int tokens_count;

        sds *splited_line = sdssplit(file_lines[i], ",", &tokens_count);

        center_x = strtof(splited_line[0], NULL);
        center_y = strtof(splited_line[1], NULL);
        center_z = strtof(splited_line[2], NULL);
        ...
        float half_face_x = strtof(splited_line[3], NULL);
        float half_face_y = strtof(splited_line[4], NULL);
        float half_face_z = strtof(splited_line[5], NULL);

        sdsfreesplitres(splited_line, tokens_count);
    ...
    free_lines_or_dir_list(file_lines);
...
void save_vtk_unstructured_grid_as_vtu(struct vtk_unstructured_grid *vtk_grid, char *filename, bool binary) {
    ...
    if(!binary) {
        ptrdiff_t num_points = arrlen(vtk_grid->points);
...
void save_vtk_unstructured_grid_as_legacy_vtk(struct vtk_unstructured_grid *vtk_grid, char *filename, bool binary) {
    ...
    ptrdiff_t num_values = arrlen(vtk_grid->values);

    for(ptrdiff_t i = 0; i < num_values; i++) {
        if(binary) {
...
bool convert_to_files_in_dir_to_vtu(const char *input_dir, const char *output_dir, const char *file_prefix, const bool animate_grid, const bool compressed, const bool plain, const bool verbose) {
    ...
    ptrdiff_t n_files = arrlen(files_in_dir);
    bool success = false;
    struct vtk_unstructured_grid *grid = NULL;

    for (int i = 0; i < n_files; ++i) {

        sds full_input_file_path = sdsnew(input_dir);
        full_input_file_path = sdscat(full_input_file_path, "/");
        full_input_file_path = sdscat(full_input_file_path, files_in_dir[i]);

        success = new_vtk_unstructured_grid_from_file(&grid, full_input_file_path, false, NULL, false, NULL, !animate_grid);

        if(!success) {
            fprintf(stderr, "Error creating grid from %s\n", full_input_file_path);
        }

        char *tmp = remove_ext(files_in_dir[i], '.', '/');
        sds new_vtk_file = sdsnew(tmp);
        free(tmp);

        new_vtk_file = sdscat(new_vtk_file, ".vtu");
        sds full_output_file_path = sdsnew(output_dir);
        ...
        if(compressed) {
            ...
            save_vtk_unstructured_grid_as_vtu(grid, full_output_file_path, !plain);
        }

        sdsfree(new_vtk_file);
        sdsfree(full_input_file_path);
        sdsfree(full_output_file_path);

        if(animate_grid) {
            free_vtk_unstructured_grid(grid);
            grid = NULL;
        }
    }
    ...
    free_lines_or_dir_list(files_in_dir);
